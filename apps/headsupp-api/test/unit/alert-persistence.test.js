...
test('allows same-bucket re-alert after a completed resume action', async () => {
  const alerts = [];
  const deliveries = [];
  const enqueued = [];
  const controls = [];
      ...
      return {
        bind(...params) {
          return {
            ...
            params,
            async run() {
              if (/INSERT INTO watch_action_controls/.test(sql)) {
                controls.push({
            ...
            async first() {
              if (/FROM signals/.test(sql)) return { signal_key: 'forecast.revenue.pace' };
              if (/FROM watches/.test(sql)) return { watch_id: params[0] };
              if (/FROM watch_action_controls/.test(sql)) {
                const [workspaceId, channelId, watchId, signalId] = params;
                const relevant = controls
                  .filter(
                    (control) =>
                      control.workspace_id === workspaceId
                      && control.channel_id === channelId
                      && control.action_type === 'resume'
                      && control.status === 'completed'
                      && (
                        (control.target_type === 'watch' && control.target_id === watchId)
                        || (control.target_type === 'signal' && control.target_id === signalId)
                      ),
                  )
                  .sort((a, b) => Date.parse(b.created_at) - Date.parse(a.created_at));
                return { latest_resume_at: relevant[0]?.created_at || null };
              }
              return null;
            },
            async all() {
  ...
  await db
    ...
    .bind(
      ...
      null,
      null,
      '2026-05-24T10:05:05.000Z',
      '2026-05-24T10:05:05.000Z',
    )
    .run();

  const resumed = await persistAlertWithDeliveries({
    db,
    queue,
    watch: { ...watch, id: 'watch_pace_warning', watch_id: 'watch_pace_warning', signal_id: 'sig_pace' },
    evaluation: { threshold: 85, fields: common.fields },
    decision: { action: 'alert', severity: 'warning', current_value: 64 },
    input: common.input,
    now: '2026-05-24T10:05:10.000Z',
  });

  assert.equal(resumed.deliveries[0].status, 'pending');
  assert.equal(resumed.suppressed_deliveries.length, 0);
  assert.equal(enqueued.length, 2);
});
